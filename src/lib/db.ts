...
import * as C from '@screeps/common/lib/constants';
...
export function dbEnvSadd(name, data, cb) {
    try {
        if (!_.isArray(data)) {
            data = [data];
        }
...
export function dbEnvSmembers(name, cb) {
    try {
        const env = db.getCollection('env');
        const values = env.get(1) || { data: {} };
        if (values && values.data && values.data[name]) {
...
export function dbResetAllData(cb) {
    try {
